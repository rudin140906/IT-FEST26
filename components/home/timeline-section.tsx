...
export default function TimelineSection() {
  ...
  useEffect(() => {
    ...
    async function loadTimeline() {
      try {
        const res = await fetch(apiPath("/timeline"), { signal: controller.signal });
        if (res.ok) {
  ...
  return (
    <section id="timeline" className="relative max-w-6xl mx-auto px-4 md:px-6 py-20 overflow-hidden">
      ...
      <div className="text-center mb-16 relative z-10">
        <span className="font-pixel text-xs text-pink tracking-widest uppercase">◆ AGENDA ACARA</span>
      ...
      <div className="relative">
        ...
        <div className="space-y-10 md:space-y-12">
          {items.map((item, index) => {
            ...
            const badgeBg =
              item.badgeColor === "cyan" ? "bg-cyan text-ink" : item.badgeColor === "yellow" ? "bg-yellow text-ink" : "bg-pink text-ink";

            const badgeBorderColor =
              item.badgeColor === "cyan" ? "border-cyan" : item.badgeColor === "yellow" ? "border-yellow" : "border-pink";
            ...
            return (
              <div key={`timeline-${item.id}-${index}`} className="relative flex items-center z-10 group">
                ...
                <div className={`w-full pl-14 md:pl-0 md:w-1/2 ${isEven ? "md:pr-12 md:mr-auto text-left" : "md:pl-12 md:ml-auto text-left"}`}>
                  <div className={`relative border-3 border-ink shadow-hard bg-navy-800 p-5 md:p-6 hover:shadow-hard-lg hover:-translate-y-1 transition-all duration-300 ${badgeBorderColor} border-t-4`}>
                    <div className={`absolute -top-3 left-4 px-2.5 py-0.5 text-[10px] font-pixel border-2 border-ink shadow-hard-sm ${badgeBg} uppercase font-bold`}>
                      {item.category || "AGENDAR"}
                    </div>

                    {item.imageUrl && (
                      <div className="mt-3 mb-2 w-full h-32 relative border-2 border-ink bg-navy-900 overflow-hidden">
                    ...
                    <h3 className="font-pixel text-xs md:text-sm text-cream leading-relaxed mt-2 group-hover:text-yellow transition-colors">{item.title}</h3>

                    <div className="mt-4 pt-3 border-t border-cream/10 flex items-center gap-2 text-xs md:text-sm font-pixelify font-bold text-cream/90 justify-start">
