  ...
  return (
    <div className="group relative border-3 border-ink shadow-hard bg-navy-700 overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-hard-lg" style={{ transform: index % 2 === 0 ? "rotate(-1deg)" : "rotate(1deg)" }}>
      ...
      <div className="relative h-56 flex items-center justify-center border-b-3 border-ink overflow-hidden bg-navy-800">
        <div className={`absolute w-40 h-40 rounded-full blur-[50px] ${glowClasses[competition.color]}`}/>
        <Image
          src={competition.mascot}
      ...
      <div className="p-6">
        <span className={`inline-block font-pixel text-[9px] border-3 border-ink px-2 py-1 mb-3 ${badgeClasses[competition.color]}`}>
          KOMPETISI
        </span>
        ...
        <div className="flex flex-wrap gap-3">
          <a href={competition.registerUrl} className="press-btn border-3 border-ink shadow-hard-sm bg-yellow text-ink font-mono text-xs font-bold px-4 py-2">
            DAFTAR
          </a>
          <a href={competition.guidebookUrl} target="_blank" rel="noopener noreferrer" className="press-btn border-3 border-ink shadow-hard-sm bg-transparent text-cream font-mono text-xs font-bold px-4 py-2">
            GUIDEBOOK
          </a>
        </div>
      </div>
