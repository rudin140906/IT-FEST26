...
export default function TimelineSection() {
  return (
    <section className="relative max-w-6xl mx-auto px-4 md:px-6 py-20 overflow-hidden">
      ...
      <div className="relative">
        ...
        <div className="space-y-10 md:space-y-12">
          {timelineData.map((item, index) => {
            const isEven = index % 2 === 1; // Even index (1, 3, 5, 7) on the left, Odd (0, 2, 4, 6) on the right for alternating desktop layout
            ...
            return (
              ...
              >
                ...
                <div
                  className={`absolute left-5 md:left-1/2 -translate-x-1/2 w-10 h-10 md:w-12 md:h-12 border-3 border-ink shadow-hard-sm ${badgeBg} font-pixel text-sm md:text-base font-bold flex items-center justify-center rounded-full z-20 group-hover:scale-110 transition-transform duration-300`}
                >
                  {item.id}
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <div
                      className={`absolute -top-2 left-4 px-2.5 py-0.5 text-[10px] font-pixel border-2 border-ink shadow-hard-sm ${badgeBg}`}
                    >
                      {item.category}
                    </div>

                    {/* Title */}
                    <h3 className="font-pixel text-sm md:text-base text-cream leading-relaxed mt-2 group-hover:text-yellow transition-colors">
                      {item.title}
                    </h3>
